...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train baseline gait model")
    ...
    model = build_model(config, variant="baseline").to(device)
    ...
    for step, batch in enumerate(train_loader, start=1):
        optimizer.zero_grad()
        total_loss, metrics = compute_losses(model, batch, losses, config, device, variant="baseline")
        total_loss.backward()
    ...
    save_checkpoint(model, optimizer, scheduler, iterations, eval_metrics, config, save_dir="checkpoints", filename="baseline_smoke.pth")
